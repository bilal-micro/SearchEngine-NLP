...
class Engine:
    ...
    def add_new_product(product):
        embedding = model.encode([product.title], convert_to_numpy=True)  
        embedding_str = " ".join(map(str, embedding))  
        with open("product_vectors.txt", "w", encoding="utf-8") as f:
            f.write(f"{product.id},{product.titlte},{embedding_str}\n")
    ...
    def search_product(data , k = 5):
        query_vec = model.encode([data], convert_to_numpy=True)
